...
- **Effect Evidence and Contract Boundary**: Gait carries versioned `effect_snapshot` before/after evidence for bounded Postgres, filesystem, HTTP, and resource lifecycles and grades typed `effect_contract` `expect`, `forbid`, and `invariant` predicates purely. `pass`, `fail`, and `inconclusive` are deterministic; grading never executes an external effect.
