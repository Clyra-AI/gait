...
- Release tags publish a separate release-owner authoritative evidence bundle generated from the peeled commit with a domain-separated key derived from immutable release identity (never a checked-in private key). Its manifest and standalone assets bind the release tag, artifact/schema/key digests, and require `authoritative=true`, `fixture_only=false`, `development_signing=false`, and `quarantine=false`; verify it with `go run ./scripts/action_contract_authoritative_bundle_generator --verify <bundle.zip> --checksums <signed-checksums.txt>`.
