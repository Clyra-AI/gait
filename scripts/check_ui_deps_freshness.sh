...
for pkg in "${PACKAGES[@]}"; do
  ...
  latest="$(node -e 'const {execFileSync}=require("node:child_process"); const [pkg, current] = process.argv.slice(1); const npmBin = process.platform === "win32" ? "npm.cmd" : "npm"; const out = execFileSync(npmBin, ["view", `${pkg}@^${current}`, "version", "--json"], {encoding: "utf8"}).trim(); const parsed = JSON.parse(out); process.stdout.write(Array.isArray(parsed) ? parsed[parsed.length - 1] : parsed);' "$pkg" "$normalized_current")"
