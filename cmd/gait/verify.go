...
func printUsage() {
	...
	fmt.Println("  gait job submit --id <job_id> [--policy <policy.yaml>|--policy-digest <sha256>] [--identity <id>] [--json] [--explain]")
	...
	fmt.Println("  gait containment stop --proposal proposal.json --activation activated.json --trace trace.json --trace-public-key key --activation-public-key key --evaluation-time RFC3339 --private-key key --journal lifecycle.jsonl --receipt-out receipt.json --identity identity --boundary-id boundary --adapter-identity adapter [--allow-development-signing] [--json]")
	fmt.Println("  gait contract validate --proposal <artifact.json> [--evaluation-time <rfc3339>] [--json]")
	fmt.Println("  gait contract classify (--proposal <artifact.json>|--action <runtime-action.json>|--input <classification-input.json>) [--json]")
	...
	fmt.Println("  gait contract explain [--proposal <artifact.json>|--action <runtime-action.json>|--input <classification-input.json>] [--json]")
	fmt.Println("  gait contract activate --proposal <artifact.json> --selection <manifest.json> --policy-digest sha256:<hex> --principal <ref> --authority-ref <ref> --target <target> --environment <env> --mode context_only|enforce_floor|required --private-key <key> --valid-from <rfc3339> [--valid-until <rfc3339>] [--out <activated.json>] [--lifecycle-out <lifecycle.jsonl>] [--overwrite] [--json]")
